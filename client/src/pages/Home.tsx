...
type View = "overview" | "book" | "trips" | "drivers" | "profile" | "admin";
...
function DriverDashboard() {
  return <div className="driver-dashboard"><div className="driver-hero"><div><span className="eyebrow">DRIVER MODE · ONLINE</span><h2>Your next great<br /><i>trip is close.</i></h2><p>Keep 95% of every fare. RideFlow keeps the other 5% visible, always.</p><button className="secondary-button light-button"><Gauge size={16} /> Go offline</button></div><div className="driver-pulse"><div className="pulse-core"><CarFront size={34} /></div><span>4 riders nearby</span></div></div><div className="driver-stats"><Metric label="Today's earnings" value="KSh 0" delta="New driver account" positive={false} /><Metric label="Your take-home" value="KSh 0" delta="After 5% fee" positive={false} /><Metric label="Acceptance" value="0%" delta="No requests yet" positive={false} /></div><div className="driver-columns"><div className="request-card empty-driver-card"><div className="subhead"><span><span className="eyebrow">NEW REQUESTS</span><b>No ride requests yet</b></span><span className="request-time">Ready when you are</span></div><p className="operations-empty">Go online and complete driver verification to receive nearby requests. Real fares and commission splits will appear here after your first completed ride.</p></div><div className="earnings-card"><div className="subhead"><span><span className="eyebrow">FEE BREAKDOWN</span><b>This week</b></span><MoreHorizontal size={17} /></div><div className="earnings-number">KSh 0</div><div className="earnings-bar"><span style={{ width: "0%" }} /></div><div className="earnings-labels"><span>Gross fares <b>KSh 0</b></span><span>5% to RideFlow <b>−KSh 0</b></span></div><p className="fine-print"><ShieldCheck size={13} /> Your exact fee will be shown on every completed trip.</p></div></div></div>;
}

function Trips({ setView }: { setView: (view: View) => void }) {
  return <div className="page-content simple-page"><div className="page-title-row"><div><span className="eyebrow">YOUR TRIPS</span><h1>Every ride,<br /><i>in one place.</i></h1><p>Receipts, shared trips, and lost-item help without the hunt.</p></div><button className="primary-button" onClick={() => setView("book")}>Book again <ArrowUpRight size={17} /></button></div><div className="trip-list"><div className="operations-empty trip-empty-state"><div className="trip-icon muted"><Route size={17} /></div><b>No trips yet</b><span>Your completed rides, receipts, shared trips, and lost-item help will appear here after your first booking.</span></div></div><div className="lost-item-banner"><div className="feature-icon"><Search size={18} /></div><div><b>Left something behind?</b><span>Track a lost item with the driver who had it.</span></div><button className="secondary-button" onClick={() => toast("Lost item flow opened — demo mode")}>Find an item <ArrowUpRight size={15} /></button></div></div>;
}
...
function OperationsDashboard() {
  const operations = trpc.admin.operations.useQuery(undefined, { refetchInterval: 15_000 });
  const data = operations.data;
  if (operations.isLoading) return <div className="page-content simple-page"><div className="operations-card operations-loading">Loading live operations…</div></div>;
  if (operations.isError) return <div className="page-content simple-page"><div className="operations-card operations-error"><b>Operations feed unavailable.</b><span>Check the database connection and try again.</span><button className="secondary-button" onClick={() => operations.refetch()}>Retry</button></div></div>;
  return <div className="page-content simple-page operations-page"><div className="page-title-row"><div><span className="eyebrow">ADMIN OPERATIONS</span><h1>See what’s<br /><i>moving.</i></h1><p>Authenticated presence and recent application activity, refreshed every 15 seconds.</p></div><div className="operations-live-badge"><span className="live-dot" /> Live monitor</div></div><div className="metrics-row operations-metrics"><Metric label="Total accounts" value={String(data?.counts.totalUsers ?? 0)} delta="Database users" positive={false} /><Metric label="Online now" value={String(data?.counts.onlineUsers ?? 0)} delta="Heartbeat in last 90 sec" positive={false} /><Metric label="App events" value={String(data?.recentActivity.length ?? 0)} delta="Recent activity" positive={false} /></div><div className="operations-grid"><section className="operations-card"><div className="subhead"><span><span className="eyebrow">PEOPLE IN RIDEFLOW</span><b>Who is online</b></span><UsersRound size={18} /></div><div className="operations-list">{data?.users.length ? data.users.map(person => <div className="operations-row" key={person.id}><div className={person.presenceStatus === "online" ? "presence-avatar online" : "presence-avatar"}>{(person.name ?? person.email ?? "U").slice(0, 1).toUpperCase()}</div><div className="operations-person"><b>{person.name ?? "Unnamed user"}</b><small>{person.email ?? "No email"} · {person.role}</small></div><div className="operations-state"><span className={person.presenceStatus === "online" ? "status-dot online" : "status-dot"} />{person.presenceStatus === "online" ? "Online" : person.presenceStatus ?? "Offline"}<small>{person.currentView ?? "No active view"}</small></div></div>) : <div className="operations-empty">No signed-in accounts yet. New users will appear here after their first authenticated session.</div>}</div></section><section className="operations-card"><div className="subhead"><span><span className="eyebrow">ACTIVITY FEED</span><b>What’s happening</b></span><ActivityIcon size={18} /></div><div className="operations-list">{data?.recentActivity.length ? data.recentActivity.map(event => <div className="activity-row" key={event.id}><span className="activity-mark"><Zap size={14} /></span><div><b>{event.summary}</b><small>{event.eventType} · {new Date(event.createdAt).toLocaleString()}</small></div></div>) : <div className="operations-empty">No activity recorded yet. Presence heartbeats and important actions will appear here.</div>}</div></section></div></div>;
}

function PaymentMethodSelector() {
  const [method, setMethod] = useState("cash");
  const options = [["cash", "Cash", Banknote, "Pay after the ride"], ["mpesa", "M-Pesa", Smartphone, "STK Push at checkout"], ["paypal", "PayPal", WalletCards, "Use your PayPal wallet"], ["card", "Cards", CreditCard, "Visa, Mastercard, and more"]] as const;
  return <div className="payment-method-panel"><div className="subhead"><span><span className="eyebrow">PAYMENT METHOD</span><b>Choose how you’ll pay.</b></span><CreditCard size={17} /></div><div className="payment-method-grid">{options.map(([value, label, Icon, detail]) => <button type="button" key={value} className={method === value ? "payment-method selected" : "payment-method"} onClick={() => { setMethod(value); toast.success(`${label} selected for future rides.`); }}><span className="payment-method-icon"><Icon size={17} /></span><span><b>{label}</b><small>{detail}</small></span>{method === value && <Check size={15} />}</button>)}</div><p className="fine-print"><LockKeyhole size={13} /> Payment details are only collected when the corresponding provider is configured.</p></div>;
}

function Profile({ isAdmin = false }: { isAdmin?: boolean }) {
  return <div className="page-content simple-page"><div className="page-title-row"><div><span className="eyebrow">PROFILE &amp; SETTINGS</span><h1>Your ride,<br /><i>your rules.</i></h1><p>Preferences that travel with you.</p></div><Avatar initials="AM" color="#f5dfcc" accent="#935438" /></div><div className="profile-grid"><div className="profile-card"><div className="profile-avatar"><span>AM</span><button className="icon-button"><Plus size={15} /></button></div><h3>Alex Morgan</h3><span>alex.morgan@email.com</span><PaymentMethodSelector /><ProfileStoragePanel /><div className="profile-divider" /><button className="settings-row"><CreditCard size={17} /><span><b>Payment</b><small>Choose a method above</small></span><ChevronDown size={16} /></button><button className="settings-row"><ShieldCheck size={17} /><span><b>Safety contacts</b><small>2 people can follow your trips</small></span><ChevronDown size={16} /></button></div><div className="preferences-card"><div className="subhead"><span><span className="eyebrow">DEFAULT PREFERENCES</span><b>Set once. Ride easy.</b></span><Settings2 size={17} /></div>{[["Quiet rides", MessageCircle, "On"], ["Cool cabin", Thermometer, "On"], ["Share trips automatically", UsersRound, "Off"]].map(([label, Icon, value]) => <div className="preference-setting" key={label as string}><span className="preference-setting-icon"><Icon size={17} /></span><span><b>{label as string}</b><small>{value === "On" ? "Applied to every ride" : "Choose contacts when booking"}</small></span><span className={value === "On" ? "switch on" : "switch"}><span /></span></div>)}</div></div>{isAdmin && <AdminPanel />}</div>;
}
